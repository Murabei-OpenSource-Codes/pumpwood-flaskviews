...
classmethods; the first argument for each should respect the convention
...
navigation on the sidebar. During calls to endpoint information,
this attribute will be passed to i18n for translation.

##### dimensions [dict]:
Dictionary of tag/value; this will be registered at the model class
route.
...
##### broadcast [bool]:
When ``True`` (default), save, delete, and action endpoints schedule
...
in Pumpwood. This is informational data and does not verify if the
model actually exists on Pumpwood.
...
This ensures that the main object can still be serialized and returned to
